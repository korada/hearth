...
const acceptedXMLContentTypes = ['application/xml+fhir', 'application/fhir+xml', 'application/xml', 'text/xml', 'application/fhir+xml; charset=utf-8']
...
switch (config.getConf('server:fhirVersion')) {
  case 'stu3':
    fhir = new FHIR(new ParseConformance(false, FhirVersions.STU3))
    break
  case 'r4':
    fhir = new FHIR()
    break
...
const contentTypeNegotiation = (req, res, next) => {
  ...
  if (req.rawBody && req.get('content-type') && acceptedXMLContentTypes.includes(req.get('content-type'))) {
    var jsonObj = fhir.xmlToJson(req.rawBody)
...
const outcomeHandler = (req, res, next) => {
  ...
  if (res.locals.outcome.resource) {
    ...
    let tranformedData = res.locals.outcome

    // transform content into XML as specified by content-type
    if (acceptedXMLContentTypes.includes(res.get('content-type'))) {
      tranformedData = fhir.jsonToXml(JSON.stringify(res.locals.outcome))
    }
